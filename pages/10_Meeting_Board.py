...
def _apply_meeting_mode_css() -> None:
    st.markdown(
        _html(
            """
            <style>
            /* Production view: hide Streamlit's default chrome so Meeting Board feels like an internal system. */
            #MainMenu, footer, [data-testid="stToolbar"], [data-testid="stDecoration"], [data-testid="stStatusWidget"] {
                visibility: hidden !important;
                height: 0 !important;
            }

            /* More compact page header for Meeting Board. */
            .zt-header-grid {
...
with filter_col5:
    ...
    st.button("Reset", use_container_width=True, on_click=_reset_meeting_filters, help="Reset Meeting Board filters.")
...
selected_index = st.selectbox(
    ...
    help="Select one item from the current Meeting Board filter result. The selected item opens automatically below.",
)
