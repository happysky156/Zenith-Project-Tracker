...
render_page_header(
    ...
    "Read-only ISO-style quality process view for EHS bulk order control, template download and process mapping.",
)
...
def _safe_records(process_code: str, limit: int = 300) -> tuple[list[dict[str, Any]], str]:
    try:
        if process_code == "QP-01":
            ...
            return rows, "Sales Board active project records mapped as RFQ-stage source data."
        ...
        if process_code == "QP-05":
            return [], "No dedicated Complaint / CAPA extension exists yet. Phase 1 shows process design only."
...
def _select_preview_columns(process_code: str, rows: list[dict[str, Any]]) -> list[str]:
    preferred = {
        "QP-01": ["project_id", "project_name", "client_code", "category", "priority", "status", "reference_link"],
...
def _render_quick_actions(process_code: str, rows: list[dict[str, Any]]) -> None:
    ...
    st.caption("Downloads are read-only and do not write to the database.")
    ...
    export_rows = rows if rows else [
        {
            ...
            "phase_1_note": "No mapped business records available yet. This export confirms the read-only process view structure.",
        }
    ]
    ...
    st.info(
        "Input and import stay in Import Center / later extension modules. Phase 1 keeps this page read-only."
    )
...
def _render_records_view(process_code: str) -> None:
    ...
    if not rows:
        st.warning("No mapped records are available for this process in Phase 1.")
        return rows
...
def _render_overview() -> None:
    ...
    st.write(
        "This page creates a single read-only view for five formal quality processes and one supplier management view. "
        "It is designed for a small trading company: simple controls, clear owners, template downloads and mapped records from existing modules."
    )
    ...
    st.markdown("### Phase 1 boundary")
    st.success("No database schema changes. No import logic changes. No Sales / Operation business logic changes.")
    st.markdown(
        "- Template downloads are generated from central definitions.\n"
        "- Process records are mapped from existing modules where possible.\n"
        "- AI-assisted summaries and impact assessments are design-ready but not connected in Phase 1, so there is no automatic AI cost."
    )
...
def _render_history() -> None:
    ...
    st.write(
        ...
        "timestamps, rejection comments and change impact assessment. Phase 1 is read-only design."
    )
