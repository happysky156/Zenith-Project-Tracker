...
st.set_page_config(
    page_title="AI Assistant Center - Zenith Project Tracker",
    page_icon="🔎",
    layout="wide",
)
...
render_page_header(
    "AI Assistant Center",
    "Read-only natural-language search over Sales, Operation, Dashboard, Project Details, Meeting Mode, Supplier Details, Price Comparison, Index Center, Order Details, and project history records.",
)
...
st.markdown(
    _html(
        """
        <div class="zpa-note">
        This assistant only searches current system records, including the new Supplier Details, Price Comparison, Index Center and Order Details modules. It does not change database records, does not update Meeting Prep, and uses the existing Sales Board / Dashboard order-link rule for order association questions.
        </div>
        """
    ),
    unsafe_allow_html=True,
...
with left_col:
    ...
    question = st.text_area(
        ...
        placeholder=(
            ...
            "- Show supplier quotation records for SDG-26-013\n"
            "- What is the latest USD/CNY index?\n"
            "- 哪些项目还没有订单？"
        ),
    ...
    if st.button(
        ...
        try:
            ...
            st.error(f"AI Assistant Center failed: {exc}")

    if st.button("Clear AI Assistant Center Result", use_container_width=True):
...
with right_col:
    ...
    with tab_supplier:
        _render_records_by_module(records_frame, "Supplier Details")

    with tab_price:
        _render_records_by_module(records_frame, "Price Comparison")

    with tab_index:
        _render_records_by_module(records_frame, "Index Center")

    with tab_order_details:
        _render_records_by_module(records_frame, "Order Details")
