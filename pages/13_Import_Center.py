...
render_page_header("Import Center", "Controlled Excel imports, templates and import history.")
...
def _render_template_downloads() -> None:
    ...
    tab_core, tab_process = st.tabs(["Core / Extension Templates", "Process Templates"])

    with tab_core:
        names = available_template_names()
        core_names = [name for name in names if name not in {"Index Config", "Daily Market Indices", "Index Snapshot", "Index Alert Rules", "Index Alert Events", "Freight Index"}]
        index_names = [name for name in names if name not in core_names]

        with st.expander("Business data templates", expanded=True):
            cols = st.columns(2)
                ...
                with cols[idx % 2]:
                    try:
                        ...
                        st.download_button(
                            ...
                            key=f"download_template_core_{idx}_{name}",
                            use_container_width=True,
                        )
                    except Exception as exc:
                        st.error(f"Could not generate {name}: {exc}")

        with st.expander("Index / quotation reference templates", expanded=False):
            cols = st.columns(2)
                ...
                with cols[idx % 2]:
                    try:
                        ...
                        st.download_button(
                            ...
                            file_name=template_file_name(name),
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key=f"download_template_index_{idx}_{name}",
                            use_container_width=True,
                        )
                    except Exception as exc:
                        st.error(f"Could not generate {name}: {exc}")
    ...
    with tab_process:
        ...
        with st.expander("Process control templates", expanded=True):
            cols = st.columns(2)
                ...
                with cols[idx % 2]:
                    try:
                        ...
                        st.download_button(
                            ...
                            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                            key=f"download_process_template_{idx}_{name}",
                            use_container_width=True,
                        )
                    except Exception as exc:
                        st.error(f"Could not generate {name}: {exc}")
    ...
    st.markdown(
        _html(
            ...
            <div class='zi-workflow-card'>
                ...
                <div class='zi-workflow-text'>
                    This import writes to controlled extension tables and keeps core Sales / Operation workflow unchanged.
                </div>
            </div>
            """
        ),
